...
/// @copyright 1997,1998,2015,2026 Hiroshi Kuno. MIT License
...
protected:
	...
	Degree za;		///< 一般歳差(J2000.0) ζA
	Degree Za;		///< 一般歳差(J2000.0) ΖA
	Degree ta;		///< 一般歳差(J2000.0) θA
    Degree Pa;		///< 一般歳差(J2000.0) ρA
	...
	// 太陽の平均黄経、地心距離
	Degree Ls;		///< 瞬時の平均春分点による太陽の黄経.
	double Rs;		///< 太陽の地心距離[AU]

	// 太陽の地心幾何学赤道座標[AU] ※光行差含まず
	Vec3 sunJ;		///< 太陽のJ2000.0の赤道座標[AU]
	Vec3 sunQ;		///< 太陽の瞬時の平均赤道座標[AU]
	...
	void calcMat2();		///< calc()が計算した天体位置角度から、歳差・章動・赤道／黄道座標変換行列を計算する.
...
public:
	...
	/// @brief 天文経緯度を設定する.
	/// @details 地方恒星時や高度方位変換に使用するので、極運動も含めた瞬時の
	/// 天文経緯度を設定する必要がある。
	...
	/// 測地経緯度で作成されており、天文経緯度とは最大30″のずれがある。
	...
	/// @brief 地心平均赤道座標の観測位置を得る.
	/// @details getLocation()をグリニジ平均恒星時だけ回転した値である
