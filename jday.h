/// @file
/// ユリウス日(暦日)クラスの定義モジュール.
/// @author hkuno9000@gmail.com
/// @copyright 1997,1998,2000,2026 Hiroshi Kuno. MIT License
...
@details
	...
	計算可能範囲は ±LONG_MAX年であるが、数千万年を超える範囲を計算しても
...
### ユリウス暦(Julian calendar)：
	4で割り切れる年を閏年とする暦法。
	ローマのジュリアス・シーザーが制定し、BC.45年1月1日から施行した。
...
public:
	...
	//----- J2000.0 --------------------------------------------------
	/// J2000.0(2000年1月1日12:00UT)を元期とする経過日数を求める.
	/// @return J2000.0 を元期とする経過日数.
	long jd2000() const {
		return _jd - AD2000_1_1_12UT;
	}
	/// J2000.0(2000年1月1日12:00UT)を元期とする経過日数を求める.
	/// @param y, m, dd グレゴリオ暦の年月日[世界時]
	/// @return J2000.0 を元期とする経過時間.
	static double jd2000(int y, int m, double dd) {
		Jday w(y, m, 0);
		return w.jd2000() + dd - 0.5;	// 正午が日の変わり目なので 0.5日補正する
	}
	/// J2000.0(2000年1月1日12:00UT)を元期とするユリウス年数を求める.
	/// @param y, m, dd グレゴリオ暦の年月日[世界時]
	/// @return J2000.0 を元期とするユリウス年数.
	static double jy2000(int y, int m, double dd) {
		return jd2000(y, m, dd) / 365.25;
	}
	/// J2000.0(2000年1月1日12:00UT)を元期とするユリウス世紀を求める.
	/// @param y, m, dd グレゴリオ暦の年月日[世界時]
	/// @return J2000.0 を元期とするユリウス世紀数.
	static double jc2000(int y, int m, double dd) {
		return jd2000(y, m, dd) / 36525;
	}

	//----- グレゴリオ暦の日付設定・取得 ----------------------------
	/// グレゴリオ暦の日付を得る (AD1582～現代用)
	/// @param [out] year, month, day グレゴリオ暦の年月日.
	/// @param [out] dayOfWeek 曜日. 0=日、1=月、2=火 ... 6=土.
	void getGdate(int& year, int& month, int& day, int& dayOfWeek) const;

	/// グレゴリオ暦の日付を設定する (AD1582～現代用)
	/// @param year, month, day グレゴリオ暦の年月日.
	void setGdate(int year, int month, int day);

	//----- ユリウス暦の日付設定・取得 -------------------------------
	/// ユリウス暦の日付を得る (BC4713～AD1582 の期間用)
	/// @param [out] year, month, day ユリウス暦の年月日.
	/// @param [out] dayOfWeek 曜日. 0=日、1=月、2=火 ... 6=土.
	void getJdate(int& year, int& month, int& day, int& dayOfWeek) const;

	/// ユリウス暦の日付を設定する (BC4713～AD1582 の期間用)
	/// @param year, month, day ユリウス暦の年月日.
