...
/// 3月1日からの通日(3月1日 = 0)を得る.
/// @param [in,out] y 年. 1,2月は前年に補正される. 月が12以上なら翌年に補正される.
/// @param [in] m 月 [1...12]
/// @return 通日値 [0...336] 2月29日を考慮しているため、3月1日からの通日が336まである. 336 に 29 を加算すると 365 となり、閏年の最終通日になる.
static int
...
static inline constexpr int day_of_week_from_jd(long jd)
{
	int w = (int)((jd + 1) % 7);
	if (w < 0) w += 7;
	return w;
